...
namespace StorybrewScripts
{
    public class Trademark : StoryboardObjectGenerator
    {
        ...
        public void trademark(int startTime)
        {
            for (var i = 0; i < 24; i++)
            {
                OsbSprite sprite = GetLayer("").CreateSprite($"sb/tm/box{i}.png", OsbOrigin.Centre);

                var scale = 0.6f;
                sprite.Scale(startTime, scale);
                sprite.Move(startTime, new Vector2(383, 240));

                sprite.StartLoopGroup(startTime - 4000, Random(2, 5));
                sprite.Fade(0, Random(80, 100), 0, 1);
                sprite.EndGroup();

                sprite.StartLoopGroup(startTime - 500, Random(2, 5));
                sprite.Fade(0, Random(20, 80), 1, 0);
                sprite.EndGroup();
                Log(i);
            }
        ...
        public void trademarkWithSFX(int startTime)
        {
            var delay = 0;

            var bitmap = GetMapsetBitmap("sb/tm2/p.png");
            var bitmapLogo = GetMapsetBitmap("sb/tm2/tochi.png");
            var bg = GetLayer("").CreateSprite("sb/tm2/p.png", OsbOrigin.Centre, new Vector2(320, 240));
            var logo = GetLayer("").CreateSprite("sb/tm2/tochi.png", OsbOrigin.Centre);
            var logo2 = GetLayer("").CreateSprite("sb/tm2/tochi2.png", OsbOrigin.Centre);
            var mask = GetLayer("").CreateSprite("sb/tm2/p.png", OsbOrigin.Centre);
            var mask2 = GetLayer("").CreateSprite("sb/tm2/p.png", OsbOrigin.Centre);
            // var star = GetLayer("").CreateSprite("sb/tm2/s.png", OsbOrigin.Centre);
            var sound = GetLayer("").CreateSample("sb/tm2/voicetag2.ogg", startTime - 10000, 90);
